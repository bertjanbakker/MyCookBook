...
class ItemEntry(object):
	...
	def __repr__(self):
		r"""the usual"""
		_result = []
		_result.append("item: %s" % self.item)
		_result.append("start: %s" % self.start)
		_result.append("finish: %s" % self.finish)
		_result.append("result: %s" % self.result)
		return "\t".join(_result)
...
class MultiProcessQueue(object):
	...
	def __init__(self, _itemhandler, _qsize=multiprocessing.cpu_count()):
		...
		self.processes = [self._create_process(procnum) for procnum in range(self.qsize)]
